...
func Import(ctx context.Context, config ImportConfig) error {
	...
	documentCount, segmentCount, formCount, err := streamDocuments(ctx, db, config.Documents)
	if err != nil {
		return err
	}
	if documentCount == 0 {
	...
	if err := writeMetadata(ctx, db, config, primarySHA, documentCount, segmentCount, formCount); err != nil {
		return err
	}
...
func streamDocuments(ctx context.Context, db *sql.DB, source io.Reader) (int, int, int, error) {
	tx, err := db.BeginTx(ctx, nil)
	if err != nil {
		return 0, 0, 0, err
	}
	defer tx.Rollback()
	insertDocument, err := tx.PrepareContext(ctx, `INSERT INTO documents (dictionary_id, entry_id, scope, headword, english_text, candidate_text, definition_text, chinese_text, section, part, owner_id, path_json, weight) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
		return 0, 0, 0, err
	}
	...
	for lineNumber := 1; ; lineNumber++ {
		...
		if len(line) > maxLineBytes {
			return 0, 0, 0, fmt.Errorf("projection line %d exceeds %d bytes", lineNumber, maxLineBytes)
		}
		if len(line) > 0 {
			...
			if len(line) > 0 {
				...
				if err != nil {
					return 0, 0, 0, fmt.Errorf("projection line %d: %w", lineNumber, err)
				}
				if document.EntryID < lastEntry {
					return 0, 0, 0, fmt.Errorf("projection line %d is out of order by entryId", lineNumber)
				}
	...
	return count, segmentCount, formCount, nil
}
...
func writeMetadata(ctx context.Context, db *sql.DB, config ImportConfig, primarySHA string, documentCount, segmentCount, formCount int) error {
	tx, err := db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	values := map[string]string{
		...
		"segment_count": strconv.Itoa(segmentCount), "form_count": strconv.Itoa(formCount),
	}
	for _, key := range []string{"schema_version", "projection_version", "normalizer_version", "primary_sha256", "source_version", "document_count", "segment_count", "form_count"} {
